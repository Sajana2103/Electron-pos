...
class SettingsDAO {
    ...
    static async setClientInfo(syncData) {
        ...
        try {
            ...
            console.log('clientINfo Promise', doc)


            let response = await db.put({ _id: doc._id, _rev: doc._rev, ...syncData, remoteDB: remoteDB })
            console.log('RESPONSE', response)
            return { success: response }

        } catch (err) {
            if (err.name === 'not_found') {
                let clientInfoDoc = { _id: 'clientInfo', ...syncData }
                let res = await db.put(clientInfoDoc)
                console.log('not_found', res)
                if (res.ok) {
                    return res
                } else {
                    console.log(err)
