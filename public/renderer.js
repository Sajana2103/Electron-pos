...
ipcRenderer.on('bill-window', async (event, arg) => {
  ...
  arg.data.map((item) => {
    ...
    extras = item.extras.map((extra, idx) => {
      return (
        `<div class="small-text" style={{ display: 'grid', gridTemplateColumns: 'repeat(2,auto)', alignItems: 'center', textAlign: 'center' }}>
          <p>${extra.name}</p>
          <p style="text-align:right;">${extra.price}</p>
        </div>`
      )
    })
    }
    if(!item.extras || !item.extras.length){
      console.log('else extra')
      extras = `<div class="tiny-text" >- - -</div>`}

    div.innerHTML = (`<p class="small-text" style="margin-top:2px;" >${item.quantity}</p>${extras}
